...
const AppContent: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const savedAccounts = await api.getData('accounts');
        const savedTemplates = await api.getData('templates');
        const savedRecipientLists = await api.getData('recipientLists');
        const savedCampaigns = await api.getData('campaigns');
  ...
  useEffect(() => {
    if (isLoaded) {
      api.saveData('accounts', accounts);
    }
  }, [accounts, isLoaded]);

  useEffect(() => {
    if (isLoaded) {
      api.saveData('templates', templates);
    }
  }, [templates, isLoaded]);

  useEffect(() => {
    if (isLoaded) {
      api.saveData('recipientLists', recipientLists);
    }
  }, [recipientLists, isLoaded]);

  useEffect(() => {
    if (isLoaded) {
      api.saveData('campaigns', campaigns);
    }
  }, [campaigns, isLoaded]);
...
const AuthGate: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    api.checkSession().then(ok => setStatus(ok ? 'authenticated' : 'anonymous'));
  }, []);

  if (status === 'checking') return null;
  if (status === 'anonymous') return <Login onSuccess={() => setStatus('authenticated')} />;
  return <>{children}</>;
};
